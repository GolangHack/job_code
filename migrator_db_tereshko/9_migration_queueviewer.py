...
def _getServices():
    return (db.Program
            .select()
            .where(db.Program.program_type == 1))


def _createServiceMarkSymbolVariables():
    for service in _getServices():
        var_id = uuid()
        db.Variable.create(
            id=var_id,
            name='_'.join(['service', str(service.number), 'MarkSymbol']),
            value='',
            default_value='',
            type='str',
            description='Символ для обозначения сервиса на экране очереди',
            modifiable='1',
...
if __name__ == '__main__':
    if config.readConfig('../config.conf'):
        _initConnection()
        _createServiceMarkSymbolVariables()
